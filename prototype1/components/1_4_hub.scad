...
module hub_cavity(height=hub_height + .2,fs=.5) {
	$fs=fs;
	union() {
		linear_extrude(height=height, center = false, convexity = 10, twist=0) hub_cavity_profile();
		translate(v=[0,0,3.52]) rotate(a=90, v=[-1,0,0]) cylinder(h=20,r=2);
	}
}
...
module hub_screw_cavities(fs=.05) {
	for (dirX=[1, -1]) {
		for (dirY=[1,-1]) {
			translate(v=[screw_offset*dirX,screw_offset*dirY,0]) {
				screw_cavity(fs=fs);
...
module hub_mount_surface(thickness=hub_mount_thickness,fs=.5) {
	hub_mount_thickness = thickness;
	$fs=fs;
	difference() {
		cylinder(h=hub_mount_thickness, r=hub_mount_radius);
		translate(v=[0,0,-screw_head_height - .1]) hub_screw_cavities(fs=fs);
		translate(v=[0,0,-.1])  cylinder(h=hub_mount_thickness+.2,r=axle_radius);
	}
}

module hub_mount_inset_screw(thickness=hub_mount_thickness,fs=.5) {
	hub_mount_thickness = thickness;
	$fs=fs;
	difference() {
		cylinder(h=hub_mount_thickness+screw_head_height, r=hub_mount_radius);
		hub_screw_cavities(fs=fs);
		translate(v=[0,0,-.1])
...
module hub_mount_enclosed(thickness=hub_mount_thickness,fs=.5) {
	hub_mount_thickness = thickness;
	$fs=fs;
	difference() {
		cylinder(h=hub_mount_thickness+screw_head_height+hub_height-.1, r=hub_mount_radius);
		translate(v=[0,0,hub_mount_thickness+screw_head_height]) hub_cavity(fs=fs);
		hub_screw_cavities(fs=fs);
		translate(v=[0,0,-.1])
			cylinder(h=hub_mount_thickness+screw_head_height+.2, r=axle_radius);
	}
